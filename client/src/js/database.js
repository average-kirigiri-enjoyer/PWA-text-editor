...
export const putDb = async (textEditorContent) =>
{
  ...
  const store = transaction.objectStore('jate'); //connects to the JATE object store under the above-specified conditions
...
{
  const jateDB = await openDB('jate', 1); //initializes a connection to the JATE database under version 1
  const transaction = jateDB.transaction('jate', 'readonly'); //creates a new transaction instance with the JATE database with read-only privileges
  const store = transaction.objectStore('jate'); //connects to the JATE object store under the above-specified conditions
